...
public class ClassUrlValidatorTests extends TestCase {
  ...
  private int numValidSchemes = 30;
  private int numInvalidSchemes = 30;
  private int numValidDomains = 20;
  private int numValidAuthorities = 150;
  private int numInvalidAuthorities = 200;

  private String validSchemes[] = { "aaa://","aaas://","about://","acap://","acct://","acr://","adiumxtra://","afp://","afs://","aim://","appdata://","apt://","attachment://","aw://","barion://",
...
".telnet://","tftp://","things://","thismessage://","tip://","tn3270://","tool://","turn://","turns://","tv://","udp://","unreal://","urn://","ut2004://","v-event://","vemmi://",
...
".Ls0fa://","P52(Q://","Mg&e>://","PM4GS://","!(nbR://","]{:Dd://","3an0f://","HeA-F://","jEz55://","_LAvg://",
  ...
  private String validTopLevelDomains[] = {
...
private ResultPair validPaths[] = {  {"/test1", true}, {"/t123", true}, {"/$23", true}, {"/test1/", true}, {"", true}, {"/test1/file", true}, {"/t123/file", true}, {"/$23/file", true}, {"/test1//file", true}, {"/%3Fabc/files", true} };
private ResultPair invalidPaths[] = { {"/..", false},  {"/../", false}, {"/#", false}, {"/../file", false}, {"/..//file", false}, {"/#/file", false}, {"//key", false},  {":/files", false} }; 
...
private String validQueries[] = {"?", "", "?item1=value1;item2=value2", "?item1/value1?item1=value2", "?item1", "?item1?=value1", "??=?", "?item1=1+2+some+words"};
  ...
  public ResultPair[] makeValidSchemes() {
    int usedValidNumbers[numValidSchemes];
    int randomNum;
    ResultPair schemes[numValidSchemes];

//  Assign 30 invalid schemes to list of schemes, leaving 20 schemes for invalid ones
    for (int i=0; i< numValidSchemes; i++) {
      ...
      schemes[i] = {invalidSchemes[randomNum], true};
    }
    return schemes;
}

public ResultPair[] makeInvalidScheme() {
  int usedInvalidNumbers[numInvalidSchemes];
  int randomNum;
  ResultPair schemes[numInvalidSchemes];
  for (int j=0; j < numInvalidSchemes; j++) {
    do {
      ...
      schemes[j] = {invalidSchemes[randomNum], false};
    }
    return schemes;
}
...
public ResultPair[] makeFullValidDomains() {
  int numValidDomains = 50;
  int usedValidNumbers[numValidDomains];
  ResultPair fullDomains[numValidDomains];
  int randomNum;

  for (int i=0; i< numValidDomains) {
    ...
    do {
        ...
        testBuffer.append(validDomainNames[i % invalidDomainNames.length]);
        testBuffer.append(validTopLevelDomains[randomNum]);
        fullDomains[i] = {testBuffer.toString(), true};
  }
  return fullDomains;
}
...
public ResultPair[] makeFullInvalidDomains() {
  int numInvalidDomains = 100;
  int usedInvalidNumbers[numInvalidDomains];
  ResultPair fullDomains[numInvalidDomains];
  int randomNum;

  for (int i=0; i< numInvalidDomains) {
    StringBuffer testBuffer = new StringBuffer();
    // Ensure no duplicates
    do {
          randomNum = ThreadLocalRandom.current().nextInt(0, invalidTopLevelDomains.length);
        ...
        fullDomains[i] = {testBuffer.toString(), false};
  }
  return fullDomains;
}
...
public ResultPair[] makeValidAuthorityPairs(ResultPair[] validDomains) {
  ResultPair authorities[numValidAuthorities];
  ...
  for (int i=0; i < numValidAuthorities; i++) {
        ...
        // Now assign the port
          ...
          authorities[i] = {testBuffer.toString(), true};
  }
    return authorities;
}
...
public ResultPair[] makeInvalidAuthorityPairs(ResultPair[] invalidDomains, ResultPair[] validDomains) {
  ResultPair authorities[numInvalidAuthorities];
  int randomPort;
  int useValidPort;
  for (int i=0; i < numValidAuthorities; i++) {
        StringBuffer testBuffer = new StringBuffer();

        int useValidPort = (ThreadLocalRandom.current().nextInt(0,10) % 2);
  ...
  for (int i=0; i < validSchemesM.length; i++) {
    ...
    for (int j=0; j < validAuthoritiesM.length; j++) {
      ...
      for (int k=0; k < validPaths.length; k++) {
        testBuffer.append(validPaths[k]);
        ...
        for (int m=0; m < validQueries.length; m++) {
          ...
          assert(result == true);
          int queryIndex = testBuffer.lastIndexOf(validQueries[m].item);
          testBuffer.delete(queryIndex, 200);
        }
        testBuffer.delete(pathIndex, 200);
      }
      testBuffer.delete(authorityIndex, 200);
    }
    testBuffer.delete(0, 200);
  }
...
// Invalid URLs which only have invalid paths - tests should be false
  for (int i=0; i < validSchemesM.length; i++) {
    ...
    for (int j=0; j < validAuthoritiesM.length; j++) {
      ...
      for (int k=0; k < invalidPaths.length; k++) {
        testBuffer.append(invalidPaths[k]);
  ...
  for (int i=0; i < invalidSchemesM.length; i++) {
    StringBuffer testBuffer = new StringBuffer();
    testBuffer.append(invalidSchemesM[i].item);
    for (int j=0; j < ininvalidAuthoritiesM.length; j++) {
      testBuffer.append(ininvalidAuthoritiesM[j].item);
      int authorityIndex = testBuffer.lastIndexOf(ininvalidAuthoritiesM[j].item);
      for (int k=0; k < invalidPaths.length; k++) {
        testBuffer.append(invalidPaths[k]);
        int pathIndex = testBuffer.lastIndexOf(invalidPaths[k].item);
        for (int m=0; m < invalidQueries.length; m++) {
          testBuffer.append(validQueries[m]);
          boolean result = urlVal.isValid(testBuffer.toString());
          assert(result == false);
          int queryIndex = testBuffer.lastIndexOf(validQueries[m].item);
...
public void manualValidTests() {
      ...
      assertTrue(urlVal.isValid("http://www.ietf.org/rfc/rfc2396.txt"));
      assertTrue(urlVal.isValid("ldap://[2001:db8::7]/c=GB?objectClass?one"));
      assertTrue(urlVal.isValid("mailto:John.Doe@example.com"));
      assertTrue(urlVal.isValid( "news:comp.infosystems.www.servers.unix"));
      assertTrue(urlVal.isValid("tel:+1-816-555-1212"));
      assertTrue(urlVal.isValid("telnet://192.0.2.16:80/"));
      assertTrue(urlVal.isValid("urn:oasis:names:specification:docbook:dtd:xml:4.1.2"));
